...
# ============================================================
# TELEGRAM BOT TOKEN (از Environment Variable)
# ============================================================
...
def check_github_repo(repo: str):
    ...
    if response.status_code == 403:
        raise RuntimeError(
            "محدودیت درخواست گیت‌هاب (Rate Limit).\n"
            "لطفاً چند دقیقه بعد دوباره تلاش کن یا GITHUB_TOKEN را تنظیم کن."
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "railway_token":
        ...
        try:
            ...
            await update.message.reply_text(
                f"❌ Railway Token معتبر نیست.\n\n{str(e)[:2500]}"
            )
            return

        if not workspaces:
            await update.message.reply_text(
                "❌ هیچ Workspace‌ای برای این Railway Token پیدا نشد."
            )
            return
        ...
        try:
            ...
            await update.message.reply_text(
                f"❌ دریافت لیست Region‌ها ممکن نشد.\n\n{str(e)[:2500]}"
            )
            return

        if not regions:
            await update.message.reply_text(
                "❌ هیچ Region پشتیبانی‌شده‌ای پیدا نشد."
            )
            return
        ...
        for r in regions:
            ...
            name = r.get("name")
            rows.append([
                InlineKeyboardButton(f"{flag} {label}", callback_data=f"region:{name}")
            ])
    ...
    if step in ("region", "workspace", "deploying"):
        await update.message.reply_text(
            "⏳ لطفاً از دکمه‌های بالا انتخاب کن.\nبرای شروع مجدد /reset را بزن."
        )
        return
...
async def handle_region(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    picked = next(
        (r for r in session.get("regions", []) if r.get("name") == region_name),
        None,
    )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                w.get("name", "Unnamed Workspace"),
                callback_data=f"workspace:{w.get('id')}",
            )
        ]
        for w in workspaces
    ]
    ...
    await safe_edit(
        query,
        f"🌍 Region انتخاب شد:\n{flag} {label}\n(`{region_name}`)\n\n"
        f"📁 حالا Workspace موردنظر را انتخاب کن:",
        parse_mode="Markdown",
...
async def deploy_process(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        project = await asyncio.to_thread(
            create_project, token, workspace_id, project_name
        )
        ...
        await send_message(
            context, user_id, "⚙️ در حال ساخت Service (branch: main)..."
        )

        service = await asyncio.to_thread(
            create_service, token, project_id, safe_repo_name, repo
        )
        ...
        await send_message(
            context,
            user_id,
            f"🌍 در حال تنظیم Region به {region_flag} {region_label}...",
        )

        await asyncio.to_thread(
            set_region, token, service_id, environment_id, region
        )

        current_step = "verify_region"
        actual_region = await asyncio.to_thread(
            verify_region, token, service_id, environment_id
        )
        ...
        if actual_region and actual_region != region:
            await send_message(
                context,
                user_id,
                f"⚠️ هشدار: Region درخواستی `{region}` بود ولی "
                f"Railway `{actual_region}` رو ست کرد.",
            )
            logger.warning(
                "Region mismatch: requested=%s actual=%s", region, actual_region
            )
        ...
        deployment_id = await asyncio.to_thread(
            deploy_service, token, service_id, environment_id
        )
        ...
        await send_message(
            context,
            user_id,
            "⏳ Deployment در حال اجراست...\nممکن است چند دقیقه طول بکشد.",
        )

        success, status = await asyncio.to_thread(
            wait_for_deployment, token, deployment_id, None
        )
        ...
        await send_message(
            context, user_id, "🌐 Deployment موفق شد.\nدر حال ساخت Domain..."
        )

        domain = None
        try:
            domain_data = await asyncio.to_thread(
                create_domain, token, service_id, environment_id, port
            )
            if domain_data:
...
def main():
    ...
    application.add_handler(
        CallbackQueryHandler(handle_region, pattern=r"^region:")
    )
    application.add_handler(
        CallbackQueryHandler(handle_workspace, pattern=r"^workspace:")
    )
